...
title("Funcion Sobreyectiva, Ejercicio15 f(x)=tan(x), con dominio (x|x?p/2pn) y rango (-oo,+oo)");
%Valor del eje x (V.x)
xlabel("x o VALOR DEL EJE X");
%valor del eje y (V.y)
ylabel("f(x)  o VALOR DEL EJE Y");
%Dominio de la Funcion (DF)
disp ('El dominio de la funcion es: (x|x?p/2pn)');
%Rango de la Funcion (DF)
disp ('El rango de la funcion es: (-oo,+oo)');
%Tipo de la Funcion(TF)
disp('Es sobreyectiva la funcion');
